...
目前角色頁提供二十種同角色風格展示與提示詞；獨立角色生成 agent、場景
生成 agent 尚未接入。這兩個 pending 插槽不會阻擋既有的分鏡合成與
「選定後才 4K」流程。
...
1. 在角色風格櫥窗查看二十種同角色媒材，組合或複製提示詞。
2. 進入分鏡頁，上傳一張場景圖與一張角色**正面**參考圖；四視圖資產要先
   取出正面圖，不能整張直接送入目前模板。
3. 輸入合成描述，建立 1–3 張約 1 MP 候選。
4. 檢視每張候選與 seed，明確選定真正要保留的一張。
5. 需要時填寫 4K 精修描述，再把目前選定候選送入固定
   `wf10_upscale_opt2`。
...
正式生成結果。這不影響使用者自行上傳角色正面圖與場景圖完成現有分鏡。
