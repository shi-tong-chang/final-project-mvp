...
3. 現行產品的每張雙角色最終候選都由 server 自動依序執行 B1，再把該張
   B1 中間結果送入 [`wf_dual_B2.json`](./workflows/wf_dual_B2.json)；B1 與
   B2 之間沒有人工選片，也不能把 B1 中間圖當成最終候選。下節的
   `b1_winner_seed7102.png` 是來源機歷史 golden 重放 fixture，不是現行
   UI 要求使用者中途選勝者。
...
檔名中的 `966869739023339` 是來源候選／歷史命名 seed，不是 wf10 的
KSampler seed；這次 4K golden 重放實際固定使用下述 `676767676701`。
